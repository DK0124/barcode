  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      const controls = [
        ...
        labelPadding, textAlign, fontFamily,
        textAreaRatio,
      ...
      if (resetFormatBtn) {
        resetFormatBtn.addEventListener('click', function() {
          if (confirm('確定要將所有設定重置為預設值嗎？\n\n此操作無法復原。')) {
            ...
            /* 套用預設值 */
            applySavedSettings(brotherDefaults);
            ...
            /* 清除最後選擇的預設檔記錄 */
            try {
              localStorage.removeItem('brotherBarcode_lastSelectedPreset');
            } catch (e) {
      ...
      function saveSettingsToLocal(name, settings) {
        try {
          localStorage.setItem('brotherBarcode_' + name, JSON.stringify(settings));
        } catch (e) {
          console.error('無法儲存設定：', e);
          try {
            sessionStorage.setItem('brotherBarcode_' + name, JSON.stringify(settings));
          } catch (e2) {
      ...
      function getSettingsFromLocal(name) {
        try {
          let settingsStr = localStorage.getItem('brotherBarcode_' + name);
          if (!settingsStr) {
            settingsStr = sessionStorage.getItem('brotherBarcode_' + name);
          }
        ...
        if (deletePresetBtn) {
          deletePresetBtn.addEventListener('click', function() {
            ...
            if (confirm(`確定要刪除設定檔「${selectedPreset}」嗎？`)) {
              ...
              /* 刪除設定檔數據 */
              try {
                localStorage.removeItem('brotherBarcode_preset_' + selectedPreset);
                sessionStorage.removeItem('brotherBarcode_preset_' + selectedPreset);
              } catch (e) {
              ...
              if (getSettingsFromLocal('lastSelectedPreset') === selectedPreset) {
                try {
                  localStorage.removeItem('brotherBarcode_lastSelectedPreset');
                  sessionStorage.removeItem('brotherBarcode_lastSelectedPreset');
                } catch (e) {
